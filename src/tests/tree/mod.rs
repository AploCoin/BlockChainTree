...
use crate::{
    Block::{self, BasicInfo, TokenBlock, TransactionBlock, TransactionToken},
    BlockChainTree::*,
    Transaction,
};
...
fn add_funds() {
    ...
    blockchaintree
        .add_funds(sender, &(1000u64.to_biguint().unwrap()))
        .unwrap();
...
fn create_derivative_chain() {
    ...
    let result = BlockChainTree::create_derivative_chain(sender, &[0u8; 32], 1).unwrap();
    drop(result);
